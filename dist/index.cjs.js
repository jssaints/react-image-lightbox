...
function isVideo(fileSrc) {
  ...
  var videoExts = ['webm', 'mkv', 'flv', 'f4v', 'f4p', 'f4a', 'f4b', 'vob', 'ogv', 'ogg', 'drc',
  /*   'gif',
    'gifv', */
  'mng', 'avi', 'mov', 'qt', 'wmv', 'yuv', 'rm', 'rmvb', 'asf', 'amv', 'mp4', 'm4p', 'm4v', 'mpg', 'mp2', 'mpeg', 'mpe', 'mpv', 'svi', '3gp', '3g2'];
  return new RegExp("(".concat(videoExts.join('|').replace(/\./g, '\\.'), ")$")).test(fileSrc);
}
var isAudio = function isAudio(fileSrc) {
  return fileSrc && fileSrc.toLowerCase().endsWith('.mp3') || fileSrc && fileSrc.toLowerCase().endsWith('.ogg') || fileSrc && fileSrc.toLowerCase().endsWith('.wav');
};
var isPDF = function isPDF(fileSrc) {
  return fileSrc && fileSrc.toLowerCase().endsWith('.pdf');
};
var isImage = function isImage(fileSrc) {
  return fileSrc && fileSrc.toLowerCase().endsWith('.jpg') || fileSrc && fileSrc.toLowerCase().endsWith('.jpeg') || fileSrc && fileSrc.toLowerCase().endsWith('.gif') || fileSrc && fileSrc.toLowerCase().endsWith('.bmp') || fileSrc && fileSrc.toLowerCase().endsWith('.apng') || fileSrc && fileSrc.toLowerCase().endsWith('.ico') || fileSrc && fileSrc.toLowerCase().endsWith('.cur') || fileSrc && fileSrc.toLowerCase().endsWith('.jfif') || fileSrc && fileSrc.toLowerCase().endsWith('.pjpeg') || fileSrc && fileSrc.toLowerCase().endsWith('.pjp') || fileSrc && fileSrc.toLowerCase().endsWith('.webp') || fileSrc && fileSrc.toLowerCase().endsWith('.svg') || fileSrc && fileSrc.toLowerCase().endsWith('.png');
};
function isText(mimetype) {
  ...
  var result = mimetype.match(/text/g) || [];
...
function (_Component) {
  ...
  _createClass(ReactImageLightbox, null, [{
    ...
    value: function getTransform(_ref) {
      ...
      var scaleFactor = isVideo(src) || isAudio(src) || isPDF(src) ? 1 : zoom * (targetWidth / width);
      return {
  ...
  _createClass(ReactImageLightbox, [{
    ...
    value: function getBestImageForType(srcType) {
      ...
      if (this.isImageLoaded(imageSrc)) {
        ...
        fitSizes = this.getFitSizes(this.imageCache[imageSrc].width, this.imageCache[imageSrc].height, false);
    ...
    key: "loadImage",
    value: function loadImage(srcType, imageSrc, done, MimeType) {
      var _this12 = this;
      ...
      if (isText(MimeType)) {
        var value = this.makeRequest("GET", imageSrc);
        ...
        inMemoryImage.addEventListener('loadedmetadata', function () {
          // retrieve dimensions
        ...
        var _el = global.document.createElement('audio');

        _el.preload = 'auto';
        inMemoryImage = _el;
        var _self = inMemoryImage;
        inMemoryImage.addEventListener('loadedmetadata', function () {
          // retrieve dimensions
          var height = _self.videoHeight;
          var width = _self.videoWidth; // eslint-disable-next-line no-restricted-globals
    ...
    value: function loadAllImages() {
      ...
      this.getSrcTypes().forEach(function (srcType) {
        ...
        if (props[type] && !_this13.isImageLoaded(props[type])) {
          _this13.loadImage(type, props[type], generateLoadDoneCallback(type, props[type]), props["".concat(type, "MimeType")]);
        }
      });
    ...
    value: function render() {
      ...
      var addImage = function addImage(srcType, imageClass, transforms) {
        ...
        if (bestImageInfo.content) {
          ...
          images.push(React__default.createElement("div", {
            ...
            defaultValue: bestImageInfo.content,
            readOnly: true,
            style: imageStyle,
            key: _this16.props[srcType] + keyEndings[srcType]
          })));
          ...
          images.push(React__default.createElement("div", {
            ...
            style: imageStyle,
            className: "".concat(imageClass, " ril__image")
          }, React__default.createElement(reactFileIcon.FileIcon, _extends({
            size: 60,
            extension: Extension
          ...
          images.push(React__default.createElement("div", {
            key: imageSrc + keyEndings[srcType],
            className: "".concat(imageClass, " ril__image")
          }, React__default.createElement("object", {
            data: imageSrc,
            ...
            style: imageStyle,
            key: _this16.props[srcType] + keyEndings[srcType]
          }, React__default.createElement("p", null, "Alternative text - include a link ", React__default.createElement("a", {
            href: imageSrc
          }, "to the PDF!")))));
        } else {
          if (discourageDownloads) {
            ...
            images.push(isVideo(imageSrc) ? React__default.createElement("div", {
              key: imageSrc + keyEndings[srcType],
              style: imageStyle,
              className: "".concat(imageClass, " ril__image")
            }, imageClass === "ril-image-current" && React__default.createElement(videoReact.Player, {
              ref: function ref(player) {
                _this16.player = player;
              },
              autoPlay: true,
              playsInline: true,
              key: imageSrc + keyEndings[srcType]
              ...
              src: imageSrc,
              type: "video/mp4"
            }))) : React__default.createElement("div", null, isAudio(imageSrc) ? React__default.createElement("div", null, React__default.createElement("audio", {
              style: imageStyle,
              className: "".concat(imageClass, " ril__image"),
              key: imageSrc + keyEndings[srcType],
              src: imageSrc,
              ...
              controls: true
            })) : React__default.createElement("img", _extends({}, imageCrossOrigin ? {
      ...
      return React__default.createElement(Modal, _extends({
        ...
        download: true,
        key: "download",
        className: ['ril__toolbarItemChild', 'ril__builtinButton', 'ril__downloadButton'].join(' ')
